...
class OpponentPoolAgent(Agent):
    def __init__(self, anchor_agents: list[Agent] | None = None, window_size: int = 5,):
        if window_size <= 0:
            raise ValueError("window_size must be positive.")

        self.anchor_agents = (
            [RandomAgent(), TacticalAgent()]
            if anchor_agents is None
            else list(anchor_agents)
        )

        self.historical_agents: list[Agent] = []
    ...
    def _active_agents(self) -> list[Agent]:
        history = self.historical_agents[-self.window_size:]

        return self.anchor_agents + history
    ...
    def on_episode_start(self, rng: np.random.Generator) -> None:
        ...
        index = int(rng.integers(len(active_agents)))
        self.current_agent = active_agents[index]
        self.current_agent.on_episode_start(rng)

    def add_historical_agent(self, agent: Agent) -> None:
        ...
        self.historical_agents.append(agent)
